...
const ActivityMonitoring: React.FC = () => {
  ...
  const getRoleBadge = (role: string) => {
    const roleMap = {
      admin: { label: isRTL ? 'مدير النظام' : 'Admin', variant: 'destructive' as const },
      doctor: { label: isRTL ? 'دكتور جامعي' : 'Academic Expert', variant: 'secondary' as const },
      nutritionist: { label: isRTL ? 'أخصائي تغذية' : 'Nutritionist', variant: 'secondary' as const },
      user: { label: isRTL ? 'مستخدم عادي' : 'User', variant: 'outline' as const }
    };
